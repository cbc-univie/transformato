...
'''
    ...
    def _copy_files(self, intermediate_state_file_path):
        ...
        FILES = ['omm_barostat.py', 'omm_readinputs.py', 'omm_readparams.py',
                 'omm_restraints.py', 'omm_rewrap.py', 'omm_vfswitch.py', 'omm_hmr.py']
        for f in FILES:
            try:
...
'''
        ...
        for bond in view.bonds:
            ...
            if any(hasattr(atom, 'initial_type') for atom in [atom1, atom2]):
                ...
                try:
                    logger.debug('{:7} {:7} {:9.5f} {:9.5f} \n'.format(str(atom1.type),
                                                                       str(atom2.type), bond.mod_type.k, bond.mod_type.req))
                    prm_file_handler.write('{:7} {:7} {:9.5f} {:9.5f} \n'.format(
                        str(atom1.type), str(atom2.type), bond.mod_type.k, bond.mod_type.req))
            ...
            if any(hasattr(atom, 'initial_type') for atom in [atom1, atom2, atom3]):
                ...
                try:
                    prm_file_handler.write('{:7} {:7} {:7} {:9.5f} {:9.5f} \n'.format(
                        str(atom1.type), str(atom2.type), str(atom3.type), angle.mod_type.k, angle.mod_type.theteq))
                    logger.debug('{:7} {:7} {:7} {:9.5f} {:9.5f} \n'.format(str(atom1.type), str(
                        atom2.type), str(atom3.type), angle.mod_type.k, angle.mod_type.theteq))
                except AttributeError:
                    prm_file_handler.write('{:7} {:7} {:7} {:9.5f} {:9.5f} \n'.format(
                        str(atom1.type), str(atom2.type), str(atom3.type), angle.type.k, angle.type.theteq))
                    logger.debug('{:7} {:7} {:7} {:9.5f} {:9.5f} \n'.format(str(atom1.type),
                                                                            str(atom2.type), str(atom3.type), angle.type.k, angle.type.theteq))
                ...
                try:
                    for dihedral_type in dihedral.mod_type:
                        prm_file_handler.write('{:7} {:7} {:7} {:7} {:6.5f} {:9.5f} {:9.5f} \n'.format(str(atom1.type), str(
                            atom2.type), str(atom3.type), str(atom4.type), dihedral_type.phi_k, dihedral_type.per, dihedral_type.phase))
                except AttributeError:
        ...
        for impr in view.impropers:
            ...
            if any(hasattr(atom, 'initial_type') for atom in [atom1, atom2, atom3, atom4]):
                ...
                prm_file_handler.write('{:7} {:7} {:7} {:7} {:9.5f} {:9.5f} \n'.format(str(atom1.type), str(
                    atom2.type), str(atom3.type), str(atom4.type), impr.type.psi_k, impr.type.psi_eq))
