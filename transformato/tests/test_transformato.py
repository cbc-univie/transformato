...
def test_proposed_mutation():
    for conf in ['config/2oj9-test-solvation-free-energy.yaml', 'config/2oj9-test-binding-free-energy.yaml']:
        ...
        cc1 = set([0, 3, 6, 5, 4, 14, 24, 23, 26, 22, 25, 17, 16, 28, 27, 29, 46, 47, 48, 45, 41, 44, 2, 7, 11, 9, 8, 10, 13, 12, 39, 38, 36, 37, 34, 35, 30, 32, 33, 31])
        assert(set(a.get_common_core_idx_mol1()) == cc1)
        cc2 = set([0, 3, 6, 5, 4, 14, 19, 18, 21, 17, 20, 16, 15, 23, 22, 24, 43, 44, 45, 42, 40, 41, 2, 7, 11, 9, 8, 10, 13, 12, 38, 37, 35, 36, 33, 34, 29, 31, 32, 30])
    ...
    for conf in ['config/2oj9-test-solvation-free-energy.yaml', 'config/2oj9-test-binding-free-energy.yaml']:
        ...
        for a, system in zip([s1_to_s2, s2_to_s1], [s1, s2]):
            ...
            for env in system.envs:
                ...
                for idx in m.atom_idx:
                    assert(np.isclose(original_psf[env].atoms[idx+offset].charge * (1 - scale), new_psf.atoms[idx+offset].charge))
            ...
            for env in system.envs:
                ...
                for idx in m.atom_idx:
                    assert(np.isclose(original_psf[env].atoms[idx+offset].charge * (1 - scale), new_psf.atoms[idx+offset].charge, rtol=1e-03))
            shutil.rmtree(output_file_base) 
            ...
            for env in system.envs:
                ...
                for idx in m.atom_idx:
                    idxo = idx+offset
                    assert(np.isclose(original_psf[env].atoms[idxo].charge * (1 - scale), new_psf.atoms[idxo].charge, rtol=1e-03))
