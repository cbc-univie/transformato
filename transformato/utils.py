...
def load_config_yaml(workdir, system):

    with open(f"{workdir}/config/{system}.yaml", 'r') as stream:
        try:
    ...
    settingsMap['analysis_dir_base'] = f"{workdir}/analysis"
    settingsMap['data_dir_base'] = f"{workdir}/data"
